...
for BLOCK_SIZE in "${BLOCK_SIZES[@]}"; do
    for TOPK in "${TOPK_VALUES[@]}"; do
        ...
        if (( BLOCK_SIZE * TOPK <= 18192 )); then
            # 定义输出文件名
            #OUTPUT_FILE="output/L3_DOT_1_8B/Softmax/long_log/output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            #ANSWER_FILE="output/L3_DOT_1_8B/Softmax/long_ans/output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
