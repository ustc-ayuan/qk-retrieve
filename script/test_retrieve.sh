...
BLOCK_SIZES=(8)
TOPK_VALUES=(0.7)
MTS_STRATEGIES=("MAX") # "MAX" "SUM" "RRF" 
    ...
    for TOPK in "${TOPK_VALUES[@]}"; do
        for MTS_STRATEGY in "${MTS_STRATEGIES[@]}"; do
            # 检查是否满足条件：block_size * topk <= 16 * 512
                ...
                LOG_DIR="test/mm_block_size_${BLOCK_SIZE}_threshold_${TOPK}_mts_${MTS_STRATEGY}"
                OUTPUT_FILE="test/mm_output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
                ANSWER_FILE="test/mm_ans_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
