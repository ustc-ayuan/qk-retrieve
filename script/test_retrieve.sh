...
for BLOCK_SIZE in "${BLOCK_SIZES[@]}"; do
    for TOPK in "${TOPK_VALUES[@]}"; do
        # 检查是否满足条件：block_size * topk <= 16 * 512
            ...
            # 定义输出文件名
            OUTPUT_FILE="output/L3_DOT_1_8B/per-head/ada_topk_no_relu_rrf/all_log/output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            ANSWER_FILE="output/L3_DOT_1_8B/per-head/ada_topk_no_relu_rrf/all_ans/output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            #OUTPUT_FILE="output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            #ANSWER_FILE="ans_block_${BLOCK_SIZE}_topk_${TOPK}.txt"

            # 运行 Python 脚本并重定向输出
            CUDA_VISIBLE_DEVICES=1 python3 test_locom_retrieve.py --model_path "$MODEL_PATH" --block_size "$BLOCK_SIZE" --topk_threshold "$TOPK" --output_path "$ANSWER_FILE" > "$OUTPUT_FILE" 2>&1
