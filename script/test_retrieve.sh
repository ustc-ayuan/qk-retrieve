...
for BLOCK_SIZE in "${BLOCK_SIZES[@]}"; do
    for TOPK in "${TOPK_VALUES[@]}"; do
        ...
        if (( BLOCK_SIZE * TOPK <= 18192 )); then
            # 定义输出文件名
            OUTPUT_FILE="output/L3_DOT_1_8B/per-head/No_Relu_Sum/long_log/output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            ANSWER_FILE="output/L3_DOT_1_8B/per-head/No_Relu_Sum/long_ans/output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            #OUTPUT_FILE="output_block_${BLOCK_SIZE}_topk_${TOPK}.txt"
            #ANSWER_FILE="ans_block_${BLOCK_SIZE}_topk_${TOPK}.txt"

            # 运行 Python 脚本并重定向输出
            CUDA_VISIBLE_DEVICES=1 python3 test_locom_retrieve.py --model_path "$MODEL_PATH" --block_size "$BLOCK_SIZE" --topk "$TOPK" --output_path "$ANSWER_FILE" --session_cnt "$SESSION_CNT" --qa_cnt "$QA_CNT" > "$OUTPUT_FILE" 2>&1
