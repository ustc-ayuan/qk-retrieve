...
def efficient_insert(original_cache, new_elements, cache_position):
    # 原始缓存的长度
    original_length = original_cache.shape[-2]
    # 新元素的长度
    new_length = new_elements.shape[-2]
    
    # 总长度
    total_length = original_length + new_length
    
    # 创建最终缓存的索引映射
    index_map = torch.arange(total_length, device=original_cache.device)
    
    # 将新元素的插入位置标记为负数，以便后续处理
    # 生成负数标记序列: [-1, -2, ..., -n]
    neg_markers = -torch.arange(1, len(cache_position)+1, device=cache_position.device)
    # 批量更新索引映射
    index_map = index_map.index_put_(
    ...
    # 构建最终缓存
    final_cache = torch.zeros(
    (*original_cache.shape[:-2], total_length, original_cache.shape[-1]),
    device=original_cache.device,
    dtype=original_cache.dtype
    )
    
    # 填充原始缓存
    # print(f"---------------------------------------------------------- {index_map }")
    original_indices = torch.where(index_map >= 0)[0]
    final_cache[..., original_indices, :] = original_cache
    
    # 填充新元素
    new_mask = index_map < 0
    # 计算新元素索引映射：将负数标记转换为正数索引
    new_indices = (-index_map[new_mask] - 1).long()
    # 批量写入新元素
    final_cache[..., new_mask, :] = new_elements[..., new_indices, :]
...
class DynamicCache(Cache):
    ...
    def move_to_cpu(self):
        # print("move to cpu!")
        self.key_cache = [tensor.cpu() for tensor in self.key_cache]
        self.value_cache = [tensor.cpu() for tensor in self.value_cache]
    def move_to_cpu_async(self):
        # 创建一个 CUDA 流
        ...
        with torch.cuda.stream(stream):
            # 将数据传输到 CPU（确保是非阻塞操作）
            self.key_cache = [tensor.to('cpu', non_blocking=True) for tensor in self.key_cache]
            self.value_cache = [tensor.to('cpu', non_blocking=True) for tensor in self.value_cache]

        # 等待流中的操作完成
        stream.synchronize()
    def move_to_gpu(self):

        self.key_cache = [tensor.cuda() for tensor in self.key_cache]

        self.value_cache = [tensor.cuda() for tensor in self.value_cache]
    
    def mask_del_kvcache(self, idx_list):
        for layer_idx in range(len(self.key_cache)):
            # 对索引列表进行排序，以便从后向前处理（可选）
            sorted_idx_list = sorted(idx_list, reverse=True)
            # 获取原始张量的序列维度大小
            seq_length = self.key_cache[layer_idx].shape[-2]
            # 创建一个掩码，初始化为 True，表示所有位置都保留
            mask = torch.full((seq_length,), True, dtype=torch.bool, device=self.key_cache[layer_idx].device)
            # 标记需要删除的位置为 False
    ...
    def lookup_position(self):
        print(f"tensor 位置: {self.key_cache[0].device}")

    def get_cache_slice(self, start, end):
        # 创建一个新的 DynamicCache 对象
        sliced_cache = DynamicCache()
        sliced_cache._seen_tokens = end-start

        # 遍历每一层缓存
        for layer_idx in range(len(self.key_cache)):
            # 对每一层的 key 和 value 进行切片
            sliced_key = self.key_cache[layer_idx][..., start:end, :]
            sliced_value = self.value_cache[layer_idx][..., start:end, :]

            # 将切片后的 key 和 value 添加到新的缓存中
            sliced_cache.key_cache.append(sliced_key)
            sliced_cache.value_cache.append(sliced_value)

        return sliced_cache

    def append_cache(self, new_cache):
        if new_cache == None:
            return
        self._seen_tokens += new_cache._seen_tokens
        if(len(self.key_cache) == 0):
            for layer_idx in range(len(new_cache)):
                self.key_cache.append(new_cache.key_cache[layer_idx])
                self.value_cache.append(new_cache.value_cache[layer_idx])
        else:
            for layer_idx in range(len(new_cache)):
                self.key_cache[layer_idx] = torch.cat([self.key_cache[layer_idx], new_cache.key_cache[layer_idx]], dim=-2)
                self.value_cache[layer_idx] = torch.cat([self.value_cache[layer_idx], new_cache.value_cache[layer_idx]], dim=-2)
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            assert len(cache_position) == key_states.shape[-2]
            # 对key_cache进行拼接操作，将key_state插入到指定cache_positoin
            self.key_cache[layer_idx] = efficient_insert(self.key_cache[layer_idx],key_states,cache_position)
            self.value_cache[layer_idx] = efficient_insert(self.value_cache[layer_idx],value_states,cache_position)
    ...
    def append_update(
        ...
        """将 key_states / value_states 追加到 KV-Cache，并返回更新后的张量。"""
        # Update the number of seen tokens
        if layer_idx == 0:
            self._seen_tokens += key_states.shape[-2]
