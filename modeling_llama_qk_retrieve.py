...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # prefill stage retrieve related KV Cache
        retrieve_block_cnt = 0
        ...
        if(q_len > 1):
            ...
            if retrieve_kv_len > 0:
                if len(past_key_value.key_cache) <= self.layer_idx:
                    ...
                    past_key_value._seen_tokens = retrieve_k.shape[-2]
                else:
                    past_key_value.key_cache[self.layer_idx] = torch.concat(retrieve_k,past_key_value.key_cache[self.layer_idx],dim=-2)
                    past_key_value.value_cache[self.layer_idx] = torch.concat(retrieve_v,past_key_value.value_cache[self.layer_idx],dim=-2)
        ...
        if past_key_value is not None:
            ...
            key_states, value_states = past_key_value.append_update(key_states, value_states, self.layer_idx)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        """
        模型的前向传播函数，用于处理输入，并根据配置决定是否返回注意力、隐藏层状态以及缓存等信息。
        """
        # 如果没有传入就使用 config 中的默认值
        ...
        # 如果开启了梯度检查点并处于训练模式，则不能再使用 use_cache=True
        ...
        # 如果用户没有传入 inputs_embeds，就通过 embed_tokens 计算得到
        ...
        # 如果传入的 past_key_values 不是 Cache 类型，则执行兼容的处理逻辑
        ...
        if use_cache and not isinstance(past_key_values, Cache):
         ...
         # 如果没有显式传入 cache_position，则根据已经缓存的长度和这次输入的长度自动生成
        ...
        # 如果没有显式传入 position_ids，就用 cache_position 作为 position_ids
        if position_ids is None:
            position_ids = cache_position.unsqueeze(0)
        
        
        # 根据 past_key_values (cache) 动态更新因果掩码
        causal_mask = self._update_causal_mask(
            attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions
        )
        
        # 先把 inputs_embeds 赋给 hidden_states，即后面网络的输入
        hidden_states = inputs_embeds
        
        # 构建/计算位置嵌入（rotary embedding），在模型的注意力机制中会被使用
        ...
        for decoder_layer in self.layers:
            ...
            # 若开启梯度检查点，则对 decoder_layer 的前向进行检查点包装
            ...
            # 取出更新后的 hidden_states
            hidden_states = layer_outputs[0]

            # 如果使用缓存，则从 layer_outputs 中拿到新的缓存
            if use_cache:
        ...
        # 如果之前判断了要从新格式转回旧格式，在此执行
        if return_legacy_cache:
            next_cache = next_cache.to_legacy_cache()

        # 如果不以字典形式返回，则将必要内容组织成元组
        if not return_dict:
        ...
        #     self.attentiontime.append(time()-st)
        
        # -------------------------------------------------------
        # 下面的逻辑用于将新计算出的 KVCache 进行存储（仅在满足一定条件时）。
        # -------------------------------------------------------
        
        # update history tokens cnt, for position infomation
        
        '''
        if input_ids.shape[1] > 1 and self.topk==0:
            print("##################################")
            print(past_key_values.key_cache[0].shape)
            for i in range(self.config.num_hidden_layers):
                torch.save(past_key_values.key_cache[i],"./retrieve_pt/cnt_"+str(self.save_cnt)+"_layer"+str(i)+"_k.pt")
            for i in range(self.config.num_hidden_layers):
                torch.save(past_key_values.value_cache[i],"./retrieve_pt/cnt_"+str(self.save_cnt)+"_layer"+str(i)+"_v.pt")
            self.save_cnt = self.save_cnt + 1
            print("##################################")
        #&'''
        
        self.kv_cache_manager.history_token_cnt = self.kv_cache_manager.history_token_cnt + input_ids.shape[1]
        
        # Update tail_token & tail_ids
        self._tail_tokens = self._tail_tokens + input_ids[0].tolist()
